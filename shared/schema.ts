...
export const assets = pgTable("assets", {
  id: serial("id").primaryKey(),
  userId: integer("user_id").references(() => users.id).notNull(), // Assets belong to user
  ...
  allocation: doublePrecision("allocation").default(0), // Percentage 0-100 (from optimization)
...
export const insertOptimizationSchema = createInsertSchema(optimizationHistory).omit({ id: true, userId: true, date: true });
