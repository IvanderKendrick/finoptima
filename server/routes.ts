...
export async function registerRoutes(
  ...
  app.post(api.auth.register.path, async (req, res) => {
    try {
      ...
      const token = jwt.sign({ id: user.id, email: user.email }, process.env.SESSION_SECRET || 'secret');
  ...
  app.put(api.profile.updatePassword.path, authenticateToken, async (req: any, res) => {
    try {
  ...
  app.delete(api.assets.delete.path, authenticateToken, async (req: any, res) => {
    const id = parseInt(req.params.id);
    const existing = await storage.getAsset(id);
    if (!existing || existing.userId !== req.user.id) {
      return res.status(404).json({ message: "Asset not found" });
  ...
  app.post(api.optimization.run.path, authenticateToken, async (req: any, res) => {
    try {
      const input = api.optimization.run.input.parse(req.body);
      const userAssets = await storage.getAssets(req.user.id);
      const selectedAssets = userAssets.filter(a => input.assetIds.includes(a.id));
      ...
      const avgRisk = selectedAssets.reduce((sum, a) => sum + a.risk, 0) / count;
  ...
  app.get(api.optimization.history.path, authenticateToken, async (req: any, res) => {
    const history = await storage.getOptimizationHistory(req.user.id);
    res.json(history);
  });

  app.delete(api.optimization.deleteHistory.path, authenticateToken, async (req: any, res) => {
    const id = parseInt(req.params.id);
